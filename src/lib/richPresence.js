...
const GITHUB_LINK = `https://github.com/ItsMagisterio/YandexNextMusic_client`;
const NM_WEBSITE_LINK = `https://github.com/ItsMagisterio/YandexNextMusic_client`;
const WSPORT = 6972;
...
function updateActivity(data) {
        ...
        const rpcTitle = rpcTitleRaw || null;

        const configLargeImageUrl =
                config?.programSettings?.richPresence?.largeImageUrl;
        const resolvedLargeImageUrl = configLargeImageUrl || undefined;
        ...
        const showGithubButton = githubButton;

        const activityObject = {
                name: rpcTitle || "Yandex Next",
                type: 2,
                details: title,
                state: artist,
                largeImageKey: img,
                ...
                buttons: [
                        ...(trackButton && trackId && (!isUGCTrack || isUGCShareEnabled)
                                ...
                                : []),
                        ...(showGithubButton
                                ? [{ label: "Yandex Next Project", url: NM_WEBSITE_LINK }]
                                : []),
                ],
        };
